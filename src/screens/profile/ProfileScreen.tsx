...
import { View, StyleSheet, TouchableOpacity, ScrollView, SafeAreaView, Image, Alert, TextStyle } from 'react-native';
...
export const ProfileScreen = observer(() => {
  ...
  const renderMenuItem = (icon: string, title: string, onPress: () => void) => (
    <TouchableOpacity
      ...
      onPress={onPress}
    >
      <Typography style={StyleSheet.flatten([styles.menuItemText, { color: colors.text01 }])}>
        {title}
      </Typography>
    </TouchableOpacity>
  );
  ...
  const renderStatItem = (value: number, label: string) => (
    <View style={[styles.statisticsItem, { backgroundColor: colors.bg02 }]}>
      <Typography style={StyleSheet.flatten([styles.statisticsValue, { color: colors.text01 }])}>
        {value}
      </Typography>
      <Typography style={StyleSheet.flatten([styles.statLabel, { color: colors.text02 }])}>
        {label}
      </Typography>
    </View>
  );
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.bg01 }]}>
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={[styles.statisticsContainer, { backgroundColor: colors.bg02 }]}>
          {renderStatItem(taskStore.tasks.length, translate('profile.stats.total'))}
          {renderStatItem(taskStore.completedTasks.length, translate('profile.stats.completed'))}
          {renderStatItem(taskStore.activeTasks.length, translate('profile.stats.inProgress'))}
        </View>

        <View style={styles.menuContainer}>
          {menuItems.map((item) => renderMenuItem(item.icon, item.title, item.onPress))}
        </View>
